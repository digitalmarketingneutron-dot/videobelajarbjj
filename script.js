...
document.addEventListener('click', (e) => {
    ...
    let notifList = document.getElementById('notifList');
    if (notifContainer && notifList && !notifContainer.contains(e.target)) {
...
function setupDashboard(student, data) {
    ...
    if (kelompokElem) {
        kelompokElem.innerText = "Kelompok: " + (student.kelompokKelas ? (Array.isArray(student.kelompokKelas) ? student.kelompokKelas.join(', ') : student.kelompokKelas) : "-");
    }
...
/* RENDER NOTIFIKASI */
function renderNotifikasi(dataMaster) {
    if (!currentStudent) return;
    ...
    const allNotifs = (dataMaster && dataMaster.notifikasi) ? dataMaster.notifikasi : [];
    const myNotifs = allNotifs.filter(n => n.username === currentStudent.username);
    ...
    myNotifs.forEach(notif => {
        ...
        htmlContent += `
            <div class="${itemClass}" onclick="markAsRead(this, '${notif.waktu || ''}', '${notif.url || ''}')">
                <p style="margin: 0 0 4px 0; color: #222;">${notif.pesan || notif.title || ''}</p>
                <small style="color: #888; font-size: 11px;">${notif.waktu || notif.date || ''}</small>
            </div>
        `;
    });
